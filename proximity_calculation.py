...
def main(read_dframe):
    ...
    for i in range(len(read_dframe.index)):
        if rowcount >= 5:
            # sending the tracked objs bounding boxes
            get_tracked(get_count_tracking_prev, get_count_tracking_current)
...
def get_tracked(get_count_tracking_prev, get_count_tracking_current):
    ...
    dic_current, dic_previous = check_tracking_to_bounding_boxes_loc_fix(
        get_count_tracking_prev, get_count_tracking_current)
...
def check_tracking_to_bounding_boxes_loc_fix(get_count_tracking_prev, get_count_tracking_current):
    ...
    if get_count_tracking_current < len(copy_rdframe.index):
        ...
        lsdata_trackerId_current = eval(
            copy_rdframe.iloc[get_count_tracking_current, 1])
        lsdata_name_current = eval(
            copy_rdframe.iloc[get_count_tracking_current, 2])

        # this to get the tracking id and class name from the prev index
        lsda_trackerId_prev = eval(
            copy_rdframe.iloc[get_count_tracking_prev, 1])
        ...
        get_count_tracking_current_array_bboxes = eval(
            copy_rdframe.iloc[get_count_tracking_current, 3])
        get_count_tracking_previous_array_bboxes = eval(
            copy_rdframe.iloc[get_count_tracking_prev, 3])
